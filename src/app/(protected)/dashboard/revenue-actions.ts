...
): Promise<{ total: number; rateCop: number; count: number; error?: string }> {
  try {
    const profile = await requireProfile();
    if (!(INGRESOS_ROLES as string[]).includes(profile.role)) {
      return { total: 0, rateCop: 4000, count: 0, error: "No tienes acceso a esta información." };
    }
    ...
    if (error) return { total: 0, rateCop: 4000, count: 0, error: error.message };

    const rateCop = usdCop ?? 4000;
    const total = (data ?? []).reduce((sum, r) => sum + ingresoToUsd(r.precio_final_descuento, r.moneda, rateCop), 0);
    return { total, rateCop, count: data?.length ?? 0 };
  } catch (err) {
    return { total: 0, rateCop: 4000, count: 0, error: err instanceof Error ? err.message : "No se pudo consultar." };
  }
}
